...
abstract class DataSource extends Object
{
    ...
    {
        $db = Loader::db();
        $row = $db->GetRow('select gasID, gasHandle, pkgID, gasName from GatheringDataSources where gasID = ?',
            array($gasID));
        if (isset($row['gasID'])) {
    ...
    {
        $db = Loader::db();
        $row = $db->GetRow('select gasID, gasHandle, pkgID, gasName from GatheringDataSources where gasHandle = ?',
            array($gasHandle));
        if (isset($row['gasID'])) {
    ...
    public static function getListByPackage($pkg)
    {
        $db = Loader::db();
        $list = array();
        $r = $db->Execute('select gasID from GatheringDataSources where pkgID = ? order by gasID asc',
    ...
    {
        $env = Environment::get();
        $file = $env->getPath(DIRNAME_ELEMENTS . '/' . DIRNAME_GATHERING . '/' . DIRNAME_GATHERING_DATA_SOURCES . '/' . $this->gasHandle . '/' . FILENAME_GATHERING_DATA_SOURCE_OPTIONS,
    ...
    public static function add($gasHandle, $gasName, $pkg = false)
    {
        ...
        $db->Execute('insert into GatheringDataSources (gasHandle, gasName, gasDisplayOrder, pkgID) values (?, ?, ?, ?)',
            array($gasHandle, $gasName, $gasDisplayOrder, $pkgID));
        $id = $db->Insert_ID();
